...
{:else}
  ...
  {#if (retrieval.strategy === "vector" || retrieval.strategy === "hybrid") && retrieval.vector}
    <div class="cfg-sub" role="group" aria-label="vector">
      ...
      <label class="cfg-field"><span class="cfg-lbl">base_url <span class="cfg-q" title="向量化服务的 API 基地址；留空用内置默认" aria-label="向量化服务的 API 基地址；留空用内置默认">?</span></span><input bind:value={retrieval.vector.base_url} placeholder="(默认)" /></label>
      <label class="cfg-field"><span class="cfg-lbl">dim <span class="cfg-q" title="向量维度，需与所选模型匹配（可选）" aria-label="向量维度，需与所选模型匹配（可选）">?</span></span><input type="number" min="1" bind:value={retrieval.vector.dim} /></label>
      <label class="cfg-field"><span class="cfg-lbl">timeout_ms <span class="cfg-q" title="单次向量化请求的超时（毫秒）" aria-label="单次向量化请求的超时（毫秒）">?</span></span><input type="number" min="1" bind:value={retrieval.vector.timeout_ms} /></label>
      <label class="cfg-field"><span class="cfg-lbl">batch_size <span class="cfg-q" title="批量向量化时每批的条数" aria-label="批量向量化时每批的条数">?</span></span><input type="number" min="1" bind:value={retrieval.vector.batch_size} /></label>
    </div>
  {/if}
  {#if retrieval.strategy === "subagent" && retrieval.subagent}
    <div class="cfg-sub" role="group" aria-label="subagent">
      ...
      <label class="cfg-field"><span class="cfg-lbl">base_url <span class="cfg-q" title="LLM 服务 API 基地址；留空用默认" aria-label="LLM 服务 API 基地址；留空用默认">?</span></span><input bind:value={retrieval.subagent.base_url} placeholder="(默认)" /></label>
      <label class="cfg-field"><span class="cfg-lbl">timeout_ms <span class="cfg-q" title="单次规划请求超时（毫秒）" aria-label="单次规划请求超时（毫秒）">?</span></span><input type="number" min="1" bind:value={retrieval.subagent.timeout_ms} /></label>
      <label class="cfg-field"><span class="cfg-lbl">candidates <span class="cfg-q" title="每轮候选工具数（可选）" aria-label="每轮候选工具数（可选）">?</span></span><input type="number" min="1" bind:value={retrieval.subagent.candidates} /></label>
    </div>
  {/if}
{/if}
