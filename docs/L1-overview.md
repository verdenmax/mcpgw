...
```
   ...
   │               DiscoveryRingSink, 历史 JSONL 回放 }               │
   │  build_dashboard_router：6 个 /api/* + 3 个静态路由（零构建 SPA）│
   └─────────────────────────────────────────────────────────────────┘
```

- **能力**：6 个只读 JSON 端点 `/api/{overview,upstreams,tools,metrics,traces,metrics/history}` + 3 个静态路由
  （`/` `/app.js` `/style.css`）服务一个**零构建的 vanilla-JS SPA**（每 3s 轮询，**对所有不可信字段
  HTML 转义**——query/工具名/上游名/reason/transport/meta_tool——防存储型 XSS）。
...
cargo test --all-features   # 全部测试（225 passed / 4 ignored：catalog 4 / config 40 /
                            ...
                            #   downstream 8 + e2e(stdio) 10 + e2e(http) 5 /
                            #   dashboard 19 ·
