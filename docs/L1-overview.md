...
```
   ...
   │  build_dashboard_router：13 个 /api/* + assets::static_handler  │
   │     fallback（rust-embed 内嵌的 Svelte 5 + Vite SPA）           │
   └─────────────────────────────────────────────────────────────────┘
```

- **能力**：13 个只读 JSON 端点 `/api/{overview,upstreams,upstreams/{name},tools,tools/{name},metrics,traces,traces/{id},metrics/history,calls,calls/{id},activity,about}`
  （M3 新增 `upstreams/{name}`、`tools/{name}`、`traces/{id}` 三个下钻详情端点）
...
cargo test --all-features   # 全部测试（285 passed / 5 ignored：catalog 4 / config 45 /
                            ...
                            #   dashboard 67（含 activity 聚合 + about）·
