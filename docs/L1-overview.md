...
配置 `[dashboard].admin_token_env` 后**额外提供可选的运行时禁用写子系统 B**（仅临时 disable/enable 上游/工具、经
Bearer 鉴权；`GET /api/disabled` 开放只读），仍**不**做改配/重启/撤 key。
...
```
   ...
   │  build_dashboard_router：18 个 /api/*（14 读 + 4 admin 写）+ 静态│
   │     fallback（rust-embed 内嵌的 Svelte 5 + Vite SPA）           │
   └─────────────────────────────────────────────────────────────────┘
```

- **能力**：**18 个** `/api/*` 端点 = **14 个只读** `/api/{overview,upstreams,upstreams/{name},tools,tools/{name},metrics,traces,traces/{id},metrics/history,calls,calls/{id},activity,about,disabled}`
  （M3 新增 `upstreams/{name}`、`tools/{name}`、`traces/{id}` 三个下钻详情端点；子系统 B 新增开放只读 `/api/disabled`）
  \+ **4 个 Bearer 鉴权的写端点** `POST /api/admin/{upstreams,tools}/{name}/{disable,enable}`（仅这 4 个经
  `route_layer` 挂 `require_admin_token` 中间件；未配 `admin_token_env` 时统一返 404、不泄露存在性）
...
cargo test --all-features   # 全部测试（311 passed / 5 ignored：catalog 4 / config 47 /
                            #   retrieval 22 + caching 4 + embedder 3 + golden 1 + hybrid 6 + subagent 7 + vector 6 /
                            #   embedder(openai) 5 + chat(openai) 4 / mcpgw main 16 + cli 5 + audit 1 /
                            #   upstream 15 + 集成 11 + http_connect 1 /
                            #   metatools 4 + call_tool 4 / gateway 17 + disable 2 + rebuild 8 /
                            #   observe 11 + capture 1 /
                            #   downstream 14 + e2e(stdio) 10 + e2e(http) 5 /
                            #   dashboard 77（含 activity 聚合 + about + admin/disabled）·
