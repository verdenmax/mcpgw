...
```
   ...
   │  build_dashboard_router：11 个 /api/* + assets::static_handler  │
   │     fallback（rust-embed 内嵌的 Svelte 5 + Vite SPA）           │
   └─────────────────────────────────────────────────────────────────┘
```

- **能力**：11 个只读 JSON 端点 `/api/{overview,upstreams,upstreams/{name},tools,tools/{name},metrics,traces,traces/{id},metrics/history,calls,calls/{id}}`
...
cargo test --all-features   # 全部测试（270 passed / 5 ignored：catalog 4 / config 44 /
