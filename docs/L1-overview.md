...
- `gateway` → 依赖 `metatools`/`catalog`/`retrieval`/`upstream` + `arc-swap`/`tokio`：用 `ArcSwap` 持有快照
  ...
  **（子系统 B 扩展）** `GatewayState` 另持一个 `DisableSet`（默认空集 → 行为不变），`rebuild_snapshot` 读它在
  ingest 前**跳过被禁用上游**（连 `tools/list` 都不发）、upsert 时**跳过被禁用单工具**，使禁用项不进新快照——
  详见下文「只读可视化面板」的运行时禁用写子系统 B。
  **（子系统 C 扩展）** `GatewayState` 另加 `reconcile_upstreams(old, new, trigger) -> ReconcileSummary`：按 `name`
  做**纯三向 diff**（增/删/改），`registry.remove` 删除、复用 `connect::connect_all` 连新/改、末尾 `rebuild_snapshot`；
  best-effort（连接失败记 `connect_failures`、不中止/不回滚），供 dashboard 的在线改配做上游热重载（与禁用集天然组合）。
...
可观测性的第二块拼图：在不改动网关热路径的前提下，提供一个**默认只读**的本地 web 面板，把"网关现在聚合了哪些
上游/工具、调用量与延迟分位、最近的搜索发现"可视化出来。新增 `dashboard` crate，作为**独立 task、独立 port** 的
HTTP server 与网关同进程并发运行——它默认**只读**消费既有数据源、不参与工具路由。**默认不配置时仍纯只读**；
配置 `[dashboard].admin_token_env` 后**额外提供两个可选的 Bearer 鉴权写子系统**：**子系统 B（运行时临时禁用）**
（临时 disable/enable 上游/工具；`GET /api/disabled` 开放只读）与 **子系统 C（在线改配 + 上游热重载）**
（`GET/PUT /api/admin/config` 编辑整份 `mcpgw.toml`，`[[upstream]]` 增/删/改秒级热重载、其余字段需重启）；二者
**都不**重启进程、不撤 key。
...
```
   ...
   └──────────────────────┬───────────────────────────┘
                          │ 只读
   ┌──────────────────────▼───────────────── 独立 task / 独立 port ──┐
   │  dashboard server（默认 127.0.0.1:8971，localhost，读无鉴权）    │
   │  AppState = { GatewayState 快照/last_summary, MetricsSink 快照,  │
   │               CallRingSink, DiscoveryRingSink, 历史 JSONL 回放 } │
   │  build_dashboard_router：20 个 /api/*（14 读 + 6 admin 写）+ 静态│
   │     fallback（rust-embed 内嵌的 Svelte 5 + Vite SPA）           │
   └─────────────────────────────────────────────────────────────────┘
```

- **能力**：**20 个** `/api/*` 端点 = **14 个只读** `/api/{overview,upstreams,upstreams/{name},tools,tools/{name},metrics,traces,traces/{id},metrics/history,calls,calls/{id},activity,about,disabled}`
  （M3 新增 `upstreams/{name}`、`tools/{name}`、`traces/{id}` 三个下钻详情端点；子系统 B 新增开放只读 `/api/disabled`）
  \+ **6 个 Bearer 鉴权的 admin 端点**：子系统 B 的 `POST /api/admin/{upstreams,tools}/{name}/{disable,enable}`（4 个）
  与子系统 C 的 `GET/PUT /api/admin/config`（2 个，在线改配）；这 6 个均经 `route_layer` 挂 `require_admin_token`
  中间件、未配 `admin_token_env` 时统一返 404、不泄露存在性
  ...
  **Svelte 5 + Vite 构建、rust-embed 内嵌的多视图 hash-路由 SPA**（每 3s 轮询；Svelte `{表达式}` 自动转义、
  ...
  \+ `/api/about` About/Settings（启动时组装的非敏感生效配置/限额 + 版本/git SHA/构建时间，运行期不可变，
  **绝不**含密钥/token/env 名/值；前端 About 页分组展示）。
- **进程模型（隔离 + 默认关闭）**：作为**独立 tokio task 起在自己的端口**上（默认 `127.0.0.1:8971`，**仅
  localhost、读端点无鉴权（admin 写经 Bearer）**——非 loopback 绑定会 `warn!`）；由 `[dashboard].enabled` opt-in，**默认关闭**。监听端口
...
- **运行时禁用写子系统 B（默认关闭、opt-in）**：配置 `[dashboard].admin_token_env`（env 变量**名**，`serve`
  启动期 fail-fast 解析、**绝不存 token 本身**）后，`POST /api/admin/{upstreams,tools}/{name}/{disable,enable}` 经
  `admin.rs` 的 `require_admin_token` 中间件（常量时间 Bearer 比较，复用下游 HTTP 的 `subtle::ConstantTimeEq` 模式；
  未配 → 404、缺/错 Bearer → 401）**临时**禁用/启用一个上游 namespace 或单个工具。被禁用项经 `gateway` 的
  `DisableSet` + `rebuild_snapshot` 过滤，从下游 `search_tools`/`get_tool_details`/`call_tool` **隐藏**（隐藏式语义
  经现有代码路径达成，metatools/downstream **零改动**），随时可再启用、秒级恢复；`GET /api/disabled` 开放只读地
  反映当前禁用集（`DisabledSnapshot{upstreams,tools}`）。禁用集可经 `[dashboard].disabled_state_path` 跨重启持久化。
  About 视图新增**裸 bool** `admin_enabled`（= 是否配了 `admin_token_env`，镜像 `http_auth`，**绝不**含 env 名/token 值）。
- **在线改配 + 上游热重载写子系统 C（默认关闭、opt-in）**：复用同一 `admin_token_env` Bearer 鉴权 + 须 `serve --config X`
  知道配置路径。`GET /api/admin/config` 回当前 `mcpgw.toml` 原文本，`PUT /api/admin/config` 收**整份 TOML**→
  **严格校验**（结构 + **全部 env 引用可解析** + 至少一种 server 传输，经 `main.rs` 注入 `AppState.config_validator`）→
  **原子落盘**（`<path>.bak` 备份 → temp → `fsync` → `rename`）→ 经 `gateway::reconcile_upstreams` 对 `[[upstream]]`
  增/删/改做**热重载**（秒级、best-effort）→ 回 `ApplyResult{upstreams,needs_restart}`。其余字段
  （`retrieval`/`server`/`audit`/`dashboard`/各类密钥）**可编辑、可落盘但需重启**（仅回横幅、**不**重启进程）；
  未带 `--config` → GET/PUT 全 404；校验失败 / 空白 body → 400 不写盘；全程持 `config_write_lock` 串行。
  前端新增 token-gated **Config** 编辑视图，含**两种模式——raw（保真）+ 结构化 Form（全段、即时校验、枚举下拉）**：
  表单经前端 TOML 库（**smol-toml**）与 raw 文本**在视图切换/Save 时双向转换**（非实时联动），Form 模式 Save 把表单序列化回 TOML 后**复用同一
  `PUT /api/admin/config`**（后端零改动）。**密钥仍仅经 env 引用、绝不入响应/日志；明文 token 经非 loopback 须 TLS 前置（同子系统 B）**。
- 接口/细节见 L2/L3/L4：[dashboard L2](./L2-components/dashboard.md) · [dashboard L3](./L3-details/dashboard.md) ·
  [dashboard L4](./L4-api/dashboard.md) · 上游热重载见 [gateway L2](./L2-components/gateway.md)/[L3](./L3-details/gateway.md) ·
  配置见 [config L3](./L3-details/config.md) 的 `[dashboard]` 段。
...
cargo test --all-features   # 全部测试（325 passed / 5 ignored：catalog 4 / config 47 /
                            #   retrieval 22 + caching 4 + embedder 3 + golden 1 + hybrid 6 + subagent 7 + vector 6 /
                            #   embedder(openai) 5 + chat(openai) 4 / mcpgw main 17 + cli 5 + audit 1 /
                            #   upstream 15 + 集成 11 + http_connect 1 /
                            #   metatools 4 + call_tool 4 / gateway 18 + disable 2 + rebuild 8 + reconcile 3 /
                            #   observe 11 + capture 1 /
                            #   downstream 14 + e2e(stdio) 10 + e2e(http) 5 /
                            #   dashboard 86（含 activity 聚合 + about + admin/disabled + admin/config）·
...
- **子系统 A · 调用内容捕获 ✅** —— 每次调用把 args/result（含上游错误文本）经独立 CallContentSink 通道捕获进面板内存环（call_buffer × 2 × payload_max_bytes 上界——args/result 各封顶、重启即丢、单条 UTF-8 截断）；CallDetail 展示、列表不含；元数据 CallRecord→tracing/audit/metrics 不变（审计仍仅元数据）。并支持内容过滤（`/api/calls` 的 `q` 自由文本子串 + `arg_key`/`arg_val` 结构化递归键=值，**仅 live**；主 Calls 页与详情页 Recent-calls 均有过滤 UI）。
- **子系统 B（运行时临时禁用 + Bearer 鉴权）✅ 已完成** —— dashboard 的第一个**写**能力（dashboard 路线图 M4）：`gateway` 新增 `DisableSet`（`crates/gateway/src/disable.rs`：内存 `BTreeSet` 持禁用上游 namespace + qualified 工具名，可选 JSON 持久化——原子 `temp→fsync→rename`、best-effort（失败仅 warn、内存为权威）、坏/缺文件自愈为空集；导出 `DisabledSnapshot{upstreams,tools}`）+ `GatewayState.disabled` 字段与 `with_disabled`/`disabled`/`disabled_arc` 访问器，`rebuild_snapshot` 在 ingest **前**跳过被禁用上游（不在 `summary.ingested`、连 `tools/list` 都不发）、upsert 时跳过被禁用单工具（隐藏式语义经现有代码路径达成、metatools/downstream **零改动**，仅禁用瞬间已 in-flight 的那一次调用会跑完）。`dashboard` 新增 `admin.rs`：`require_admin_token` 中间件（未配 → 404 不泄露存在性、缺/错 Bearer → 401、常量时间 `subtle::ConstantTimeEq` 比较，镜像 downstream/http.rs）+ 4 个 handler `disable/enable_upstream/tool`（**幂等优先** → 存在性校验（上游 ∈ 配置、工具 ∈ 当前 catalog，否则 404）→ `spawn_blocking` 改集+持久化 → `await rebuild_snapshot`（rebuild 失败回 **500**）→ 回整集）+ 开放只读 `GET /api/disabled`，路由 **13 → 18**（+1 开放 GET + 4 admin POST，**仅** 4 个 admin 路由经 `route_layer` 挂中间件、开放读端点不鉴权）；`AppState` 加 `admin_token: Option<Arc<str>>`、`AboutInfo` 加裸 bool `admin_enabled`。config `[dashboard]` 加 `admin_token_env`（env 名、fail-fast）/ `disabled_state_path`（**无自动默认**、`None`=纯内存）；`mcpgw serve` 加 `resolve_admin_token`（fail-fast、gated 于 `dashboard.enabled`）并在**首次 rebuild 之前**经 `DisableSet::load_or_new` 注入已加载的禁用集（持久化禁用从启动即生效，**独立于 `dashboard.enabled`**）。前端 admin token **仅存浏览器内存**（绝不 localStorage）、复用 `DisableToggle` 开关（输入 token 才显示）、disabled 徽标源自 `/api/disabled`、新增「Disabled tools」区、About 页加 `admin_enabled` 徽标 + token 输入框。**默认不配置时行为零变化（admin 全 404、禁用集空、纯只读）；token/env 名绝不入 `/api/about`、日志或状态文件；默认策略仍 `bm25`**。
- **子系统 C（在线改配 + 上游热重载 + Bearer 鉴权）✅ 已完成** —— dashboard 的第二个**写**能力（dashboard 路线图 M5，复用 M4 鉴权）：`gateway` 新增 `GatewayState::reconcile_upstreams(old, new, trigger) -> ReconcileSummary`（`crates/gateway/src/lib.rs`：纯函数 `plan_upstream_reconcile` 按 `name` 做**三向 diff**——removed→`registry.remove`、added·changed→复用 `upstream::connect::connect_all`、unchanged→不动连接——末尾单次 `rebuild_snapshot`；**best-effort**：连接失败记 `connect_failures`、不中止其它/**不回滚**已落盘配置，纯 no-op（无增删）提前返回不 rebuild；`ReconcileSummary{added,removed,reconnected,connect_failures}` 里 `added`/`reconnected` 是**意图**，须与 `connect_failures` 交叉看才是真正生效集；rebuild 仍应用 M4 禁用过滤，故热加的禁用上游仍隐藏）。`dashboard` 新增 `admin_config.rs`：`GET /api/admin/config`（回 `ConfigView{path,content}`，当前 `mcpgw.toml` 原文本）+ `PUT /api/admin/config`（收完整 TOML → **严格校验**（结构 + **所有 env 引用可解析** + 至少一种 server 传输，经注入的 `config_validator`）→ **原子落盘**(`atomic_write`：备份 `<path>.bak` → temp → `fsync` → `rename`，出错清 temp，经 `spawn_blocking`) → `reconcile_upstreams` 上游热重载 → `restart_diff` 算非上游需重启段 → 回 `ApplyResult{upstreams: ReconcileSummary, needs_restart}`）；全程持 `config_write_lock`（tokio `Mutex`）串行；`config_path==None`→**404**、校验失败/**空白 body**→**400 不写盘**；`applied_upstreams` 基线**排除**连接失败者（故再 PUT 会重试失败上游）。**全文编辑**（无 `toml_edit`，注释/排版天然保留）；校验逻辑**注入**——`main.rs` 的 `validate_config_text`（复用启动期 `resolve_api_keys`/`resolve_admin_token`/`validate_upstream_http_env`/`build_backends`）经 `AppState.config_validator` 注入，**无 dashboard→main 依赖**。`AppState` 加 6 字段（`config_path`/`config_validator`/`config_write_lock`/`boot_config`/`applied_upstreams`/`rebuild_trigger`）；`main.rs` 的 `run_serve(cfg, config_path)` + `prepare_state` 返回 rebuild-trigger clone 接线。**热 vs 需重启**：仅 `[[upstream]]` 增/删/改秒级热重载，`[retrieval]`/`[server]`/`[audit]`/`[dashboard]` 及各类密钥**可编辑、可落盘但需重启**（dashboard **不**重启进程，只回横幅提示）。前端新增 token-gated **Config** 视图（`<textarea>` + Save → 显示 `ApplyResult`：上游 added/removed/reconnected/**connect_failures** 区分 + **需重启**横幅）；admin token 仍**仅存浏览器内存**。`.gitignore` 加 `*.bak`。**默认不配置 `admin_token_env`/`--config` 时 `/api/admin/config` 全 404、行为零变化；密钥仅经 env 引用、绝不入日志/响应；明文 token 经非 loopback 的部署前提同 M4（须 TLS 反代前置）；默认策略仍 `bm25`**。
