...
```
   ...
   │  build_dashboard_router：12 个 /api/* + assets::static_handler  │
   │     fallback（rust-embed 内嵌的 Svelte 5 + Vite SPA）           │
   └─────────────────────────────────────────────────────────────────┘
```

- **能力**：12 个只读 JSON 端点 `/api/{overview,upstreams,upstreams/{name},tools,tools/{name},metrics,traces,traces/{id},metrics/history,calls,calls/{id},activity}`
...
cargo test --all-features   # 全部测试（283 passed / 5 ignored：catalog 4 / config 45 /
