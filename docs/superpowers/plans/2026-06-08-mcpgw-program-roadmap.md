...
- **M2.T5 — SubagentStrategy（可选）**：用小模型（Haiku/Flash）选工具，behind config。✅ 已完成（默认仍 bm25，opt-in）
...
**Tasks**
- **M6.T1 — 结构化调用日志 + 追踪**：每次调用（上游、延迟、错误）可追踪。✅ 已完成（observe crate + TracingSink + downstream instrumentation；仅元数据，无参数/结果载荷；M6.T3 审计落盘已完成、JsonlSink 复用同一 sink 接缝；T2 用量指标、T4 code-mode 继续延后）
- **M6.T2 — 用量指标 + 导出**：按 tool/key 统计；Prometheus/OpenTelemetry 导出。
- **M6.T3 — 审计持久化**：调用与决策的审计落库。✅ 已完成（`observe` 的 `JsonlSink` + 专用 OS 线程 writer：每次元工具调用一行**仅元数据** JSONL，有界 channel 满则丢弃不阻塞、写失败自愈、关停优雅 drain+flush+fsync；`spawn_writer`/`AuditWriter`/`AUDIT_CHANNEL_CAPACITY`；配置 `[audit]` enabled/path，std-only、默认关闭；M6.T2 用量指标、T4 code-mode 继续延后）
