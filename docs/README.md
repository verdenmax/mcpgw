...
> 埋点在 `downstream::call_tool`，`mcpgw serve` 注入默认 `[TracingSink]`）**。注意
> `embedder`、`chat` 与 `observe` crate 都只有 L2 + L4 文档（无独立 L3；`observe` 的细节并入 `downstream` L3）。
> 后续里程碑（M3 OAuth/反向代理、M4 运行时密钥管理、M6.T3 审计落盘/M6.T2 用量指标等）
> 将按上述规则继续补充各层文档。
