...
`DiscoveryRingSink` ring+writer、history 限量回放的实现细节；外加**两个可选**写子系统：运行时禁用
（子系统 B，`admin.rs`）与在线改配 + 上游热重载（子系统 C，`admin_config.rs`，M5），均经 `[dashboard].admin_token_env`
开启，本文覆盖其鉴权、handler 流程、原子写与隐私边界。
...
- **独立端口、独立 task**：面板是 `mcpgw serve` 里一个**与下游 stdio/HTTP 完全分开**的 axum server，
  ...
  `with_graceful_shutdown(oneshot)`。它**默认只读** `Arc<GatewayState>` 与两个 sink、不参与 MCP 协议；写面是
  **两个可选** Bearer 鉴权子系统：**子系统 B**（运行时禁用，下文「运行时禁用写子系统」节，只改 `GatewayState.disabled`
  集并触发一次 rebuild）与 **子系统 C**（在线改配 + 上游热重载，下文「在线改配子系统」节，校验后原子落盘 + 经
  `reconcile_upstreams` 热重载上游）；二者**都不**重启进程/撤 key；未配 `admin_token_env` 时面板与历史版本一致（纯只读）。
- **localhost；读端无鉴权、写端 Bearer 鉴权**：默认绑 loopback、**读端点不挂任何 auth 层**（与下游 HTTP 的 Bearer
  鉴权不同）；仅子系统 B/C 的 6 个 admin 端点经 `require_admin_token` 中间件做 Bearer 鉴权（开放只读端点
...
- **静态交付变化**：由原先 `include_str!` 三文件（`/`、`/app.js`、`/style.css`）改为单个 `assets::static_handler`
  ...
  `/api/*` 端点数由 **13 → 20 个**：原 13 读 + 子系统 B 新增的 **1 个开放只读** `GET /api/disabled`（共 14 读）+
  **4 个 Bearer 鉴权写** `POST /api/admin/{upstreams,tools}/{name}/{disable,enable}`（→ 18）+ 子系统 C 新增的
  **2 个 Bearer 鉴权** `GET/PUT /api/admin/config`（→ 20；M3 详情下钻 + M1 `/api/calls`、`/api/activity`、`/api/about`
  属原 13 读）。
...
admin 层经 `route_layer` 挂在 **6 个 admin 操作**上——子系统 B 的 4 个 disable/enable POST + 子系统 C 的 `GET/PUT /api/admin/config`（开放只读端点 `GET /api/disabled` 不经过它）：
...
> **部署前提**：admin Bearer token 假定 dashboard 绑在 loopback 或由 TLS 反代前置；若绑非 loopback 地址，token 将随请求走**明文 HTTP**（`unauthenticated_public_bind` 已对非 loopback 绑定 `warn!`），生产环境务必置于 TLS 之后。
...
- admin token **只存浏览器内存**（组件状态），**绝不**写 localStorage/sessionStorage、不入 URL、不随只读请求发送——
  仅在 admin 请求（子系统 B 的 4 个 disable/enable POST + 子系统 C 的 `GET/PUT /api/admin/config`）的 `Authorization` 头里带上；刷新即清、需重新输入。
...
## 在线改配子系统（子系统 C，`admin_config.rs`，M5）

面板的**第二个写面**，同样**默认关闭**：须配 `[dashboard].admin_token_env`（复用子系统 B 的同一 Bearer 鉴权）**且**
`serve --config X`（运行时知道配置文件路径）。它让运维在浏览器里编辑**整份 `mcpgw.toml`**：`GET /api/admin/config`
回当前文本，`PUT /api/admin/config` 收完整 TOML → 严格校验 → 原子落盘(+.bak) → 上游热重载 → 回 `ApplyResult`。
**全文编辑**（不引入 `toml_edit`：前端提交整份、后端逐字写回，注释/排版是用户自己的文本、天然保留）。

### 校验逻辑注入（依赖方向）

严格校验 = **结构**（`Config::from_toml_str`：解析 + `validate`，含 `deny_unknown_fields`/必填/范围）+ **所有 env 引用可解析**
（与 `serve` 启动期同一套检查）。但 env 解析器都在 `mcpgw` bin 的 `main.rs`，dashboard 不该反向依赖 main、`config` crate
须保持**纯解析**。故 `main.rs` 把校验器 `validate_config_text` 经
`AppState.config_validator: Arc<dyn Fn(&str) -> Result<Config, String> + Send + Sync>` **注入**，内部串起
`from_toml_str` + `resolve_api_keys` + `resolve_admin_token` + `validate_upstream_http_env` + `build_backends`（含
"至少启用一种 server 传输"）。PUT handler 只调它，任一 Err → **400 + 该消息、绝不落盘**。校验逻辑留在 main、零耦合。

### `put_config` 流程（`config_path==None` 取锁前短路 404，其余持 `config_write_lock` 串行）

```text
PUT /api/admin/config  (Bearer，经 require_admin_token 中间件)
  ├─ config_path == None        → 404（serve 未带 --config，无文件可改；取锁前短路）
  ├─ 取 config_write_lock（以下校验 → 落盘 → 热重载全在锁内串行）
  ├─ body.trim().is_empty()     → 400（空/全空白 body，不写盘）
  ├─ validator(text) == Err(m)  → 400 + m（不写盘）
  └─ Ok(new_cfg):
       ① 原子落盘（spawn_blocking(atomic_write) 离开 async worker）：
              备份当前文件 → <config>.bak（best-effort，失败仅 warn）
              写同目录 temp(.tmp.{pid}) → write_all + sync_all(fsync) → rename 覆盖（同盘原子）
              出错清理 temp；落盘 IO 失败 → 500（原文件/.bak 不动）
       ② 上游热重载：gateway.reconcile_upstreams(applied_upstreams, new_cfg.upstreams, rebuild_trigger)
       ③ 更新基线 applied_upstreams = new_cfg.upstreams 里**连接成功**者（排除 connect_failures，故再 PUT 重试失败者）
       ④ needs_restart = restart_diff(boot_config, new_cfg) 的非上游差异段
       └─ 200 Json(ApplyResult{ upstreams: ReconcileSummary, needs_restart: Vec<&'static str> })
```

`config_write_lock`（`tokio::Mutex<()>`）串行化 PUT 的校验/写/reconcile 临界区（`config_path==None` 的 404 在取锁前短路），使其不并发交错；reconcile 末尾单次 rebuild
（与 gateway 既有 `rebuild_lock` 叠加）。

### 原子写 + `.bak`

`atomic_write` 每次保存前把当前文件复制为单个 `<config>.bak`（**覆盖式、非时间戳**，不做版本历史）。顺序
**备份 → temp+fsync → rename** 保证任何时刻 `config` 要么旧完整、要么新完整；崩溃不留半截（temp 出错即清）。`.bak` 已
gitignore（`*.bak`）。

### 热 vs 需重启（`restart_diff`）

- **热应用**：`[[upstream]]` 增/删/改 → `reconcile_upstreams` 的 connect/remove + rebuild，秒级生效（详见
  [gateway L3](./gateway.md) 的「上游热重载」节）。
- **需重启**：`restart_diff(boot_config, new_cfg)` 解构 `new_cfg` 逐段比 `[retrieval]`/`[server]`/`[audit]`/`[dashboard]`
  与**启动基线** `boot_config`（非"上一次 PUT"），差异段名入 `needs_restart`——这些字段**可编辑、可落盘，但重启才生效**。
  以 `boot_config` 为基线使"与正在运行的进程相比是否仍需重启"在多次 PUT 间累计准确。dashboard **不**重启/管理进程
  （它只是 serve 的子任务），`needs_restart` 只提示运维手动重启。

### best-effort 一致性与基线

落盘后某上游热应用失败时**不回滚**已落盘配置（落盘 = 用户意图），`ApplyResult` 如实报告
`upstreams.connect_failures` + `needs_restart`；`applied_upstreams` 基线只纳入**实际连上**者，故对同一份配置再次 PUT
会重试仍失败的上游而非误判 unchanged。某上游"连上但 ingest 失败/超时"经既有 Overview/Upstreams 视图（`last_summary`）
观察，不在本 PUT 响应里重复。

### 部署前提（明文 token，同 M4）

写凭据与配置文本经 admin 端点传输；admin Bearer token 假定 dashboard 绑 loopback 或由 TLS 反代前置。若绑非 loopback，
token/配置将随请求走**明文 HTTP**（`unauthenticated_public_bind` 已对非 loopback 绑定 `warn!`），生产务必置于 TLS 之后。
配置文件本身**无明文密钥**（仓库惯例全 env 引用），`GET` 原样返回；密钥仅以 env **名**出现、绝不入日志/响应。

### 前端：Config 编辑视图（token-gated）

token 非空时显示导航项 **Config**：进入 `adminGet('/api/admin/config')` 载入当前 TOML 进 `<textarea>`，**Save** →
`adminPut('/api/admin/config', text)`：200 → 展示 `ApplyResult`（上游 added/removed/reconnected/**connect_failures** 区分 +
...
- `admin_config.rs`（**M5 在线改配子系统 C**）：`get_config` 在 `config_path=None` 时 404、有路径时回 `ConfigView{path,content}`
...
- 接缝来源见：[observe-lib L4](../L4-api/observe-lib.md)（`DiscoverySink`/`DiscoveryRecord`）·
  ...
  [gateway L3](./gateway.md)（子系统 C 调用的 `reconcile_upstreams` 上游热重载）
