...
- **`resolve_api_keys(&cfg)`**：无 `[server.http]` 或其 `enabled = false` 时返回空 `Vec`（故关闭 HTTP 的配置即便
  ...
  `[[server.http.api_key]]` 的密钥，任一缺失即 `Err`。**解析出的密钥若为空/纯空白（`trim().is_empty()`）也 fail-fast 拒绝**
  （避免一个空 env 静默变成「无鉴权」）。错误消息仅含 `name`/`env` **名**，**绝不含密钥值**。
...
- `main.rs` 单测：`cli_parses_serve_subcommand`（clap 解析 `serve`）；
  ...
  `resolve_api_keys_rejects_set_but_empty_env`（set-but-empty/空白 env 也 fail-fast）、
