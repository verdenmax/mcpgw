...
| `fn build_embedder` | `(&Config) -> Result<Option<Arc<dyn retrieval::Embedder>>, String>` | 按 `retrieval.strategy` 建 embedder：`"vector"`/`"hybrid"` → 从 `api_key_env` **启动期 fail-fast** 读 key（缺失即 `Err`，消息仅含 env **名**不含值）、建 `OpenAiEmbedder` 再裹 `CachingEmbedder`（缓存跨 snapshot 重建复用）；其它（`bm25`）→ `None`；单测可达 |
