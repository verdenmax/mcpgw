...
| `fn run` | `(Cli) -> Result<(), String>` | 主逻辑：加载配置 → 执行子命令（`Serve` 起 tokio 运行时 `block_on(run_serve(cfg, cli.config.clone()))`，把 `--config` 路径透传给在线改配子系统 C） |
...
| `async fn prepare_state` | `(&Config) -> Result<(Arc<GatewayState>, mpsc::Receiver<String>, mpsc::Sender<String>, Vec<String>), String>` | `build_backends` → `GatewayState::with_backends(&cfg.retrieval.strategy, backends)` → **`.with_disabled(Arc::new(DisableSet::load_or_new(cfg.dashboard.disabled_state_path.map(PathBuf::from))))`**（子系统 B：在**首次 rebuild 之前**注入运行时禁用集；`None` path → 纯内存、坏/缺文件自愈为空集 + `warn`；**独立于 `dashboard.enabled`**，故已持久化的禁用项从启动即生效）→ `connect_all`（eager-connect 上游、装 trigger 发送端）→ 初始 `rebuild_snapshot`（**跳过被禁用上游/工具**）→ 返回状态 + worker 接收端 **+ rebuild-trigger 发送端的一份 clone（`tx.clone()`；子系统 C 用它注入 `AppState.rebuild_trigger`，使在线改配 reconcile 末尾能触发 rebuild）** + **启动连接失败的上游名字 `Vec<String>`（`csum.skipped` 的 name，供 `run_serve` 据此把 `applied_upstreams` seed 为 boot-connected 子集、排除 boot 连失败者）**；单测可达 |
...
| `fn resolve_admin_token` | `(&Config) -> Result<Option<Arc<str>>, String>` | **（子系统 B）** 解析 `[dashboard].admin_token_env` 指向的 env 得 admin Bearer token。**仅当 `dashboard.enabled`**：未配 `admin_token_env` → `Ok(None)`（admin 写 API 关闭）；env 未设 / 设了但空或全空白 → **fail-fast `Err`**（消息仅含 env **名**、不含值，镜像 `resolve_api_keys`——关闭的子系统不校验其密钥）。`Some(Arc<str>)` 注入 `AppState.admin_token`；单测 `resolve_admin_token_*`（4 例：未配/dashboard 关/读 env 成功/缺失·空·全空白 fail-fast）覆盖 |
| `fn validate_config_text` | `(&str) -> Result<Config, String>` | **（子系统 C：注入 dashboard 的在线改配校验器）** 把整份 TOML 文本做与 `serve` 启动期**同一套**严格校验：`Config::from_toml_str`（结构/`deny_unknown_fields`/范围）→ 校验**至少启用一种 server 传输**（`stdio` 或 `http.enabled`，否则 `Err`）→ `resolve_api_keys` + `resolve_admin_token` + `validate_upstream_http_env` + `build_backends`（**所有 env 引用须可解析**）。任一步 `Err` 即回带消息（仅含 env/上游/header **名**、绝不含值）。经 `Arc::new(validate_config_text) as dashboard::ConfigValidator` 注入 `AppState.config_validator`——把 env 解析逻辑留在 bin、使 `dashboard` 不反向依赖 `mcpgw`、`config` crate 保持纯解析；单测 `validate_config_text_ok_and_rejects_bad_toml_and_missing_env` 覆盖 |
| `async fn run_serve` | `(Config, Option<PathBuf>) -> Result<(), String>` | 装 stderr tracing → 校验至少启用一种传输（`stdio` 或 `http.enabled`，否则 `Err`）→ fail-fast 解析 `resolve_api_keys` + `validate_upstream_http_env` + **`resolve_admin_token`（子系统 B 的可选 admin Bearer token）** → `prepare_state`（解构 4-元组 `(state, rx, rebuild_trigger, boot_skipped)`，留 trigger clone 供子系统 C、`boot_skipped` 供 seed 排除 boot 连失败上游）→ spawn `run_rebuild_worker` → 装配观测 sinks（以 `observe::TracingSink` 打底；`Arc<[Arc<dyn observe::CallSink>]>`，stdio/http 共享）→ `cfg.audit.enabled` 时 `observe::spawn_writer(&cfg.audit.path, observe::AUDIT_CHANNEL_CAPACITY).map_err(...)?` 追加 `JsonlSink`、留 `Option<AuditWriter>`（打开文件 fail-fast）→ **`cfg.dashboard.enabled` 时**追加 `dashboard::MetricsSink` 进 sinks 切片、并建 `dashboard::CallRingSink::new(call_buffer)` 放进 `content_sinks: Arc<[Arc<dyn observe::CallContentSink>]>`（否则空切片）+ 读 `payload_max_bytes`（逐条调用**内容**通道，args/result 只入此环，喂 dashboard `AppState.calls`）；**且 `trace_queries` 时**经 `dashboard::DiscoveryRingSink::spawn(trace_buffer, trace_path)` 建发现追踪 ring + 可选 JSONL writer，装进 `discovery_sinks: Arc<[Arc<dyn observe::DiscoverySink>]>`（否则空切片）→ 预绑定 HTTP listener（`TcpListener::bind`，bind 失败即 `Err`）→ **预绑定 dashboard listener**（同样 fail-fast；非 loopback 且无 auth 时 `warn!`）→ **把 HTTP 起为带 `with_graceful_shutdown`（oneshot 驱动）的后台 task**（`build_router(..., sinks, discovery_sinks, content_sinks, payload_max_bytes)` → `axum::serve`）→ **把 dashboard 起为另一个独立 port 上的后台 task**（构造 `AppState`（**含 `admin_token`：子系统 B 的 admin 写 API 鉴权 token，`None` → 6 个 `/api/admin/*` 经中间件返回 404**；**并注入子系统 C 的 6 个字段**：`config_path`（= 传入的 `--config` 路径，`None` → `/api/admin/config` 返回 404）、`config_validator`（= `Arc::new(validate_config_text) as dashboard::ConfigValidator`）、`config_write_lock`（新建 `tokio::Mutex<()>`）、`boot_config`（= `Arc::new(cfg.clone())` 作 restart_diff 基线）、`applied_upstreams`（= **boot-connected 子集**：`cfg.upstreams` 去除 `boot_skipped` 后作 reconcile 基线，使 boot 失败上游修好后可原样 re-PUT 重试）、`rebuild_trigger`（= `prepare_state` 返回的 trigger clone））→ `build_dashboard_router(app_state, enforce_loopback_host)` → `axum::serve` + 自己的 `with_graceful_shutdown`）→ `tokio::select!` 等首个关停触发：stdio（`GatewayServer::new(state, top_k, sinks, discovery_sinks, content_sinks, payload_max_bytes)` → `serve(stdio())` → `waiting()`）/ HTTP task / dashboard task **自行结束** / `ctrl_c` → 收尾顺序：`http_shutdown_tx.send(())` → 有界 drain HTTP → `dash_shutdown_tx.send(())` → 有界 drain dashboard（`DASHBOARD_SHUTDOWN_TIMEOUT`，释放其 `AppState` 内 `DiscoveryRingSink` clone）→ `drop(sinks)` 触发审计 channel 断连 → 有界 drain 审计 writer → `drop(discovery_sinks)` + `drop(discovery_ring)` 触发发现 channel 断连 → 有界 drain 发现 writer → best-effort 拆卸上游 |
...
#### dashboard（`[dashboard].enabled`，默认只读 + 可选写子系统 B/C）
当 `[dashboard].enabled` 时，`serve` 额外把**默认只读可视化面板**起为**独立 task、独立 port**（默认
`127.0.0.1:8971`、**localhost、读端无鉴权**——非 loopback 绑定会 `warn!`）：把 `dashboard::MetricsSink` 追加进
...
`/api/about`；`git_sha`/`build_time` 来自上面的 `build.rs`），并把 `resolve_admin_token(&cfg)?` 的结果填进
**`AppState.admin_token`**（子系统 B：`Some` → 开启 4 个 Bearer 鉴权的 `/api/admin/{upstreams,tools}/{name}/{disable,enable}`
写端点；`None` → 写端点经中间件 404；开放只读 `GET /api/disabled` 始终可用）。**子系统 C（在线改配 + 上游热重载）**另注入 6 个
`AppState` 字段：`config_path`（= `serve` 的 `--config` 路径透传，决定 `GET/PUT /api/admin/config` 是否可用——`None` → 404）、
`config_validator`（= `Arc::new(validate_config_text) as dashboard::ConfigValidator`，把启动期同款 env 校验逻辑注入 dashboard、
避免反向依赖）、`config_write_lock`（`tokio::Mutex<()>` 串行化 PUT）、`boot_config`（= `Arc::new(cfg.clone())` 作 `restart_diff`
基线）、`applied_upstreams`（= **boot-connected 子集**——`cfg.upstreams` 去除 `prepare_state` 回传的 `boot_skipped`，作 `reconcile_upstreams` 基线，使 boot 连接失败的上游修好后可**原样 re-PUT 重试**而非判为 unchanged）、`rebuild_trigger`（=
`prepare_state` 返回的 trigger 发送端 clone，供 reconcile 末尾触发 rebuild）。故子系统 C 需**同时**配 `admin_token_env`
（鉴权）与 `serve --config X`（知道改哪个文件），缺 `--config` 时面板照常起、仅 `/api/admin/config` 返回 404。注：运行时禁用集 `DisableSet` 由
`prepare_state` 在更早处加载注入，**独立于本 `dashboard.enabled` 分支**（持久化禁用从启动即生效）。关停时按固定顺序：HTTP drain →
