...
| `async fn run_serve` | `(Config) -> Result<(), String>` | 装 stderr tracing → 校验至少启用一种传输（`stdio` 或 `http.enabled`，否则 `Err`）→ fail-fast 解析 `resolve_api_keys` + `validate_upstream_http_env` → `prepare_state` → spawn `run_rebuild_worker` → 装配**默认观测 sinks** `[observe::TracingSink]`（`Arc<[Arc<dyn observe::CallSink>]>`，stdio/http 共享）→ 预绑定 HTTP listener（`TcpListener::bind`，bind 失败即 `Err`）→ `tokio::select!` 并发跑 stdio（`GatewayServer::new(state, top_k, sinks)` → `serve(stdio())` → `waiting()`）/ HTTP（`build_router(..., sinks)` → `axum::serve`）/ `ctrl_c`，任一完成即统一关闭 → 收尾 best-effort `shutdown` 各上游 |
...
（处理上游 `tools/list_changed`）→ 装配**默认观测 sinks** `[observe::TracingSink]`（`Arc<[Arc<dyn observe::CallSink>]>`，
stdio 与 HTTP **共享同一切片**，故两条传输的每次元工具调用都被记为一条仅元数据的 `CallRecord` 并写成
结构化 `tool_call` tracing 事件）→ `tokio::select!` over {stdio `waiting()`、`axum::serve`、`ctrl_c`} 统一关闭，
