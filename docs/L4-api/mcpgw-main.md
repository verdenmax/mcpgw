...
#### dashboard（`[dashboard].enabled`，默认只读 + 可选写子系统 B）
...
写端点；`None` → 写端点经中间件 404；开放只读 `GET /api/disabled` 始终可用）。注：运行时禁用集 `DisableSet` 由
