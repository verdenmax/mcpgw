...
工具**排序/检索**。定义可插拔、**异步**（`#[async_trait]`）的 `RetrievalStrategy` 抽象，并提供三种实现：
...
- `EmbedderRequired(String)`：策略需要嵌入器但未提供（`"vector"`/`"hybrid"` 且 `embedder` 为 `None`）。
...
- `"hybrid"` → `HybridStrategy`，**要求** embedder，否则返回 `EmbedderRequired`；未知名 → `NotImplemented`。
...
- 逐文件 API 见 L4：[retrieval/lib.rs](../L4-api/retrieval-lib.md) ·
  ...
  [retrieval/hybrid.rs](../L4-api/retrieval-hybrid.md)
