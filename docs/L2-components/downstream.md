...
- **`api_keys` 非空 → 叠加 Bearer 鉴权层**（`from_fn_with_state` + `require_api_key`，M1-C T4）：请求须带
  `Authorization: Bearer <key>` 且 `<key>` 与某配置 key **常量时间相等**（`subtle::ConstantTimeEq`）才放行，
  缺失/错误 → **401**（不回显期望值）；`api_keys` 为空 → 不挂层、放行（依赖 localhost 绑定）。
...
- 元工具逻辑见：[metatools L2](./metatools.md) · 状态/重建见：[gateway L2](./gateway.md) ·
  调用观测见：[observe L2](./observe.md) · [observe-lib L4](../L4-api/observe-lib.md)
